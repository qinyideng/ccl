...
class WideResNetVar(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, is_remix=False, proj=False, low_dim=64):
        ...
        if self.proj:
            ...
            self.fc1 = nn.Linear(128 * widen_factor, 128 * widen_factor)
            self.relu_mlp = nn.LeakyReLU(inplace=True, negative_slope=0.1)
            self.fc2 = nn.Linear(128 * widen_factor, low_dim)
...
class build_WideResNetVar:
    def __init__(self, first_stride=1, depth=28, widen_factor=2, bn_momentum=0.01, leaky_slope=0.0, dropRate=0.0,
                 use_embed=False, is_remix=False, proj=False):
