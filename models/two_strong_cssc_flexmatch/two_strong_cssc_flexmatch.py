...
from .two_strong_cssc_flexmatch_utils import consistency_loss, Get_Scalar
...
class Two_Strong_CSSC_FlexMatch:
    def __init__(self, net_builder, num_classes, ema_m, T, p_cutoff, hard_label=True, t_fn=None, p_fn=None, it=0, num_eval_iter=1024, tb_log=None, logger=None):
        ...
        super(Two_Strong_CSSC_FlexMatch, self).__init__()
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s, x_ulb_s1) in zip(self.loader_dict['train_lb'],
            ...
            x_lb, x_ulb_w, x_ulb_s, x_ulb_s1 = x_lb.cuda(args.gpu), x_ulb_w.cuda(args.gpu), x_ulb_s.cuda(
                args.gpu), x_ulb_s1.cuda(args.gpu)
            ...
            with amp_cm():
                ...
                over_thre_features = torch.cat((features_x, f_u_s1[over_thre_mask]), 0)
                over_thre_features = torch.cat((over_thre_features, f_u_s2[over_thre_mask]), 0)
                ...
                less_thre_features = torch.cat((f_u_s1[less_thre_mask], f_u_s2[less_thre_mask]), 0)
                ...
                # The number of unlabeled high-confidence and low-confidence data in one iteration
                # multiple data augmentation of the same image is not considered
                if self.it != 0 and self.it % self.num_eval_iter == 0:
                    self.run['high_confidence_count'].log((top1_high.shape[0] - num_lb) / 2)
                    self.run['low_confidence_count'].log(top1_low.shape[0] / 2)
                ...
                if top1_low.shape[0] != 0 and top1_high.shape[0] != 0:
                    ...
                    # logits_mask3 = torch.sum(
                    #     torch.eq(top1_low.repeat(1, args.k).repeat(1, min_k_high.shape[0])
                    #              .view(top1_low.shape[0], min_k_high.shape[0], -1),
                    #              min_k_high.repeat(top1_low.shape[0], 1)
                    #              .view(top1_low.shape[0], min_k_high.shape[0], -1)), dim=2)
