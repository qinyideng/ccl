...
from models.ccl_flexmatch.ccl_flexmatch import CCL_FlexMatch
...
def main(args):
    ...
    ngpus_per_node = 2 # number of gpus of each node
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = CCL_FlexMatch(_net_builder,
                     args.num_classes,
                     args.ema_m,
                     args.T,
                     args.p_cutoff,
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            model.model = torch.nn.parallel.DistributedDataParallel(model.model, device_ids=[args.gpu],
    ...
    if args.dataset != "imagenet":
        train_dset = SSL_Dataset(args, alg='ccl', name=args.dataset, train=True,
                                 num_classes=args.num_classes, data_dir=args.data_dir)
        lb_dset, ulb_dset = train_dset.get_ssl_dset(args.num_labels)

        _eval_dset = SSL_Dataset(args, alg='ccl', name=args.dataset, train=False,
                                 num_classes=args.num_classes, data_dir=args.data_dir)
    ...
    if args.resume and Path(args.load_path).is_file():
    ...
    if not args.multiprocessing_distributed or \
        ...
        model.save_model('checkpoint.pth.tar', save_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--change', type=str, default='nochange')
